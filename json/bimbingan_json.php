<?php
	...
	if ($_POST['leveluser'] == 'admin') {
		...
		$tampil = mysql_query(" SELECT * FROM table4  WHERE (kodepembimbing1 = '" . $_POST['nik'] . "' OR kodepembimbing2 = '" . $_POST['nik'] . "') " . $cari . " ORDER BY Nama DESC LIMIT " . $offset . "," . $rows);
		$hitung = mysql_query("SELECT * FROM table4 WHERE (kodepembimbing1 = '" . $_POST['nik'] . "' OR kodepembimbing2 = '" . $_POST['nik'] . "') " . $cari . " ORDER BY Nama ");
	} else {
		$cari = ($sortby != "") ? "AND " . $sortby . " LIKE '%" . $data_cari . "%'" : "";
		$tampil = mysql_query(" SELECT * FROM table4  WHERE nim = '" . $_POST['nik'] . "' " . $cari . " ORDER BY Nama DESC LIMIT " . $offset . "," . $rows);
		$hitung = mysql_query("SELECT * FROM table4 WHERE nim = '" . $_POST['nik'] . "' " . $cari . " ORDER BY Nama ");
	}
