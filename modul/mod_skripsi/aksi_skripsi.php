...
if ($module == 'skripsi' AND $act == 'select') {
    ...
    if (mysql_num_rows($edit)) {
        $option1 = "<option value='0' > Pembimbing 1 </option>";
        $option2 = "<option value='0' > Pembimbing 2 </option>";
        while ($data = mysql_fetch_array($edit)) {
            $option1 .= "<option value='" . $data['nip'] . "' > " . str_replace("'", "", $data['nama']) . " </option>";
            $option2 .= "<option value='" . $data['nip'] . "' > " . str_replace("'", "", $data['nama']) . " </option>";
        }
        $pesan = array(
            "option1" => $option1,
            "option2" => $option2
    ...
    if (mysql_num_rows($edit)) {
        ...
        $option1 = "<option value='" . $data['kodepembimbing1'] . "' > " . str_replace("'", "", $data['pbnama1']) . " </option>";
        $option2 = "<option value='" . $data['kodepembimbing2'] . "' > " . str_replace("'", "", $data['pbnama2']) . " </option>";
        $option3 = "<option value='" . $data['konsentrasi'] . "' > " . str_replace("'", "", $data['konsentrasi']) . " </option>";
        $option4 = "<option value='' > Status </option>
                    <option value='Di ACC' " . ($data['status'] == "Di ACC" ? "selected" : "") . "> Di ACC </option>
                    <option value='Di Revisi' " . ($data['status'] == "Di Revisi" ? "selected" : "") . "> Di Revisi </option>
    ...
    if ($cek == 0) {
        ...
        if (mysql_affected_rows() > 0) {
            if ($status == TRUE) {
                ...
                $data = array("status" => 0, "msg" => "Data Berhasil Di Simpan.. Email Gagal di kirim Ke dosen Pembimbing.!");
            }
        } else {
            $data = array("status" => 1, "msg" => "Data Gagal Di Simpan");
        }
    } else {
        $data = array("status" => 1, "msg" => "Status Pengajuan Judul Skripsi Sebelumnya Masih Dalam Proses..(Tidak Bisa Menambah Data)");
    }
    ...
    if ($_SESSION['leveluser'] == 'admin') {
        ...
        $tampil = mysql_query("SELECT skripsi.judul,skripsi.status,skripsi.nomor,skripsi.nim,skripsi.nama,skripsi.tgl,pb1.nama AS pbnama1, pb2.nama AS pbnama2,skripsi.link,skripsi.kodepembimbing2 FROM table111 AS skripsi
                           LEFT JOIN table6 AS pb1 ON pb1.nip = skripsi.kodepembimbing1
                           LEFT JOIN table6 AS pb2 ON pb2.nip = skripsi.kodepembimbing2 
                           WHERE (skripsi.kodepembimbing1 = '" . $_SESSION['nik'] . "' OR skripsi.kodepembimbing2 = '" . $_SESSION['nik'] . "')
                           " . $where . " ORDER BY skripsi.tgl DESC LIMIT " . $offset . "," . $rows);
    } else {
        $where = isset($_POST['cari']) ? "AND " . $sortby . " LIKE '%" . $data_cari . "%'" : "";
        $tampil = mysql_query("SELECT skripsi.judul,skripsi.status,skripsi.nomor,skripsi.nim,skripsi.nama,skripsi.tgl,pb1.nama AS pbnama1, pb2.nama AS pbnama2,skripsi.link,skripsi.kodepembimbing2 FROM table111 AS skripsi
                           LEFT JOIN table6 AS pb1 ON pb1.nip = skripsi.kodepembimbing1
                           LEFT JOIN table6 AS pb2 ON pb2.nip = skripsi.kodepembimbing2 WHERE nim = '" . $_SESSION['nik'] . "'
                           " . $where . " ORDER BY skripsi.tgl DESC LIMIT " . $offset . "," . $rows);
    }
    $hitung = mysql_query("SELECT * FROM table111 ORDER BY tgl ");
    mysql_close();
    while ($r = mysql_fetch_array($tampil)) {
        if (($_SESSION['leveluser'] == 'admin' OR $_SESSION['leveluser'] == 'dosen') && $r['status'] != "Di Tolak") {
            if ($r['kodepembimbing2'] != $_SESSION['nik']) {
                $aksi = " <a onclick = SelectData('" . $r['nomor'] . "','" . $r['nim'] . "','?module=skripsi&act=edit') href='javascript:void(0);' data-toggle='modal' data-target='#edit' class ='glyphicon glyphicon-pencil'> </a> &nbsp;";
            }
        } else {
            $aksi = "";
        }
        $data['rows'][] = array("nomor" => $r['nomor'],
            ...
            "pbnama1" => $r['pbnama1'],
            "pbnama2" => $r['pbnama2'],
            "link" => "<a href='" . $r['link'] . "' target='_blank'>" . $r['link'] . "</a>",
            "judul" => $r['judul'],
